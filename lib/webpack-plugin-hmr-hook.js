...
HmrHook.prototype.parseModule = function(modulePath) {
  ...
  if (fileName.indexOf(".ts") > 0 || fileName.indexOf(".js") > 0)
    return "script";
...
HmrHook.prototype.debug = function(
  newModules,
  changedModules,
  removedModules,
  newModulesCount,
  ...
  Object.keys(changedModules).forEach(id => {
    console.log(
      "[plugin-hmr-hook] detected changed module: " + changedModules[id]
    );
  });
  Object.keys(removedModules).forEach(id => {
    console.log(
      "[plugin-hmr-hook] detected removed module: " + removedModules[id]
    );
  });
  //
  if (
    ...
    console.log(
      "[plugin-hmr-hook] a new bundle was compiled but nothing changed" +
