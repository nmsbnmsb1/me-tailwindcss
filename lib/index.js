...
utils.pre = function (theme, options = {}, parentConfig = undefined) {
  ...
  utils.config.future = options.future || undefined;
  utils.config.experimental = options.experimental || undefined;
  utils.config.stage = !isNaN(options.stage) ? options.stage : 4;
  return utils.config;
};
...
utils.disableAllCorePlugins = function () {
  utils.config.corePlugins = [];
};
...
utils.var = function (varName, value) {
  ...
  //使用css-varible
  // let stage = utils.config.theme['stage'] || (utils.config.theme['stage'] = {});
  // if (utils.config.stage < 4) {
  //   stage[varName] = extraValue(value);
  // } else {
  //   stage[varName] = `var(${varName})`;
  // }
};
...
utils.extraColor = function (k, defaultCssVarName, value) {
  let map = {};
  // 获取value
  let cssVarName = '';
  if (value.val) {
    cssVarName = value.var || defaultCssVarName;
    if (cssVarName.startsWith('--')) cssVarName = cssVarName.substring(2);
    value = value.val;
  }
  //$base
  ...
  if (typeof value === 'string' || typeof value === 'number') {
    ...
    if (value.length === 1) value = [0, 0, 0, 0, value[0]];
    // console.log(k, cssVarName);
    let bc = value[4]; //"#409eff"|["#409eff",.5,.1]
    if (Array.isArray(bc)) bc = bc[0];
    //map[`${k}.default`] = !cssVarName ? bc : { var: `--${cssVarName}`, val: bc };
    //
    value.forEach((cs, i) => {
      if (!cs) return;
      ...
      let id = (i + 1) * 100;
      let mod;
      if (id < 500) mod = utils.getLightModCSS;
      else if (id > 500) mod = utils.getDarkModCSS;
      let key = `${k}.${id}`;
      if (id === 500) key = `${k}.default`;
      let cssvn = cssVarName ? `--${cssVarName}-${id}` : '';
      if (cssvn && id === 500) cssvn = `--${cssVarName}`;
      //设置主色
      let cval = mod && typeof cs[0] === 'number' ? mod(bc, cs[0]) : cs[0];
      map[key] = !cssvn ? cval : { var: cssvn, val: cval };
      //设置alpha
      for (let i = 1; i < cs.length; i++) {
        let acval = utils.getAlphaModCSS(cval, cs[i]);
        let aid = cs[i].toString().replace(/\./g, '');
        let akey = `${k}.${id}-${aid}`;
        if (id === 500) akey = `${k}.${aid}`;
        let acssvn = cssVarName ? `--${cssVarName}-${id}-${aid}` : '';
        if (acssvn && id === 500) acssvn = `--${cssVarName}-${aid}`;
        map[akey] = !acssvn ? acval : { var: acssvn, val: acval };
      }
    });
  }
  //console.log(map);
...
utils.setTypeColors = function (ts) {
  for (let t in ts) {
    if (ts[t]) utils.setAllKeys(utils.colorKeys, utils.extraColor(t, t, ts[t]));
  }
};
...
utils.setTextColors = function (cs) {
  for (let k in cs) {
    if (cs[k]) utils.group(utils.keys.textColor, utils.extraColor(k, `${utils.varKeys[utils.keys.textColor]}-${k}`, cs[k]));
  }
};
...
utils.setBgColors = function (cs) {
  for (let k in cs) {
    if (cs[k]) utils.group(utils.keys.backgroundColor, utils.extraColor(k, `${utils.varKeys[utils.keys.backgroundColor]}-${k}`, cs[k]));
  }
};
...
utils.setBorderColors = function (cs) {
  for (let k in cs) {
    if (cs[k]) utils.group(utils.keys.borderColor, utils.extraColor(k, `${utils.varKeys[utils.keys.borderColor]}-${k}`, cs[k]));
  }
};

// utils.setColorVars = function (cs) {
//   for (let cssVarName in cs) {
//     let m = utils.extraColor(cssVarName, cssVarName, cs[cssVarName]);
//     for (let k in m) utils.var(m[k].var, m[k].val);
//   }
// };
