...
utils.setDefinedTextColors = function (main, regular, sub, placeholder, disabled, others) {
  let cs = { main, regular, sub, placeholder, disabled, ...others };
  for (let k in cs) {
    if (cs[k]) utils.group(utils.keys.textColor, extraColor(k, `${utils.varKeys[utils.keys.textColor]}-${k}`, cs[k]));
  }
};
utils.setDefinedBorderColors = function (main, regular, sub, disabled, others) {
  let cs = { main, regular, sub, disabled, ...others };
  for (let k in cs) {
    if (cs[k]) utils.group(utils.keys.borderColor, extraColor(k, `${utils.varKeys[utils.keys.borderColor]}-${k}`, cs[k]));
  }
};
utils.setDefinedBgColors = function (main, regular, sub, disabled, others) {
  let cs = { main, regular, sub, disabled, ...others };
  for (let k in cs) {
    if (cs[k]) utils.group(utils.keys.backgroundColor, extraColor(k, `${utils.varKeys[utils.keys.backgroundColor]}-${k}`, cs[k]));
  }
};
