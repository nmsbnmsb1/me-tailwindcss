  ...
  if (!configPath) {
    ...
    for (let k of utils.spacingKeys) {
      ...
      for (let sk in utils.config.theme[k]) {
        if (sk === 'screen') {
          utils.config.theme[utils.keys.spacing]['w-screen'] = '100vw';
          utils.config.theme[utils.keys.spacing]['h-screen'] = '100vh';
        } else {
...
utils.setDefinedTypeColors = function(primary, secondary, success, danger, warning, info, others) {
  ...
  for (let t in ts) utils.setAllColors(extraColor(t, t, ts[t]));
};
